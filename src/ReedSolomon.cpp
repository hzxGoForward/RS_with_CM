...
void ReedSolomonEncoder::EncodeOneDataBlock(
    ...
    void* recoveryBlock)         // Output recovery block
{
    ...
    {
        gf256CtxPtr->gf256_addset_mem(recoveryBlock, originals[0].Block, originals[1].Block, params.BlockBytes);
        for (int j = 2; j < params.OriginalCount; ++j)
        {
            gf256CtxPtr->gf256_add_mem(recoveryBlock, originals[j].Block, params.BlockBytes);
        }
        return;
    }
    ...
    // For other rows:
    {
        ...
        // Unroll first operation for speed
        {
            ...
            gf256CtxPtr->gf256_mul_mem(recoveryBlock, originals[0].Block, matrixElement, params.BlockBytes);
        }
        ...
        for (int j = 1; j < params.OriginalCount; ++j)
        {
            ...
            gf256CtxPtr->gf256_muladd_mem(recoveryBlock, matrixElement, originals[j].Block, params.BlockBytes);
        }
    }
}

int ReedSolomonEncoder::Encode(
    EncoderParams params, // Encoder params
    DataBlock* originals,      // Array of pointers to original blocks
    void* recoveryBlocks)        // Output recovery blocks end-to-end
{
    ...
    {
        EncodeOneDataBlock(params, originals, (params.OriginalCount + block), recoveryBlock);
    }
...
bool ReedSolomonDecoder::Initialize(EncoderParams& params, DataBlock* blocks)
{
    Params = params;
...
void ReedSolomonDecoder::DecodeM1()
{
    ...
    for (int ii = 0; ii < OriginalCount; ++ii)
    {
        ...
        else
        {
            ...
            gf256CtxPtr->gf256_add2_mem(outBlock, inBlock, inBlock2, Params.BlockBytes);
            inBlock = nullptr;
        }
    }
    ...
    if (inBlock)
    {
        gf256CtxPtr->gf256_add_mem(outBlock, inBlock, Params.BlockBytes);
    }
...
void ReedSolomonDecoder::GenerateLDUDecomposition(uint8_t* matrix_L, uint8_t* diag_D, uint8_t* matrix_U)
{
    ...
    for (int k = 0; k < N - 1; ++k)
    {
        ...
        const uint8_t D_kk = gf256CtxPtr->gf256_add(x_k, y_k);
        const uint8_t L_kk = gf256CtxPtr->gf256_div(g[k], D_kk);
        const uint8_t U_kk = gf256CtxPtr->gf256_mul(gf256CtxPtr->gf256_div(b[k], D_kk), gf256CtxPtr->gf256_add(x_0, y_k));

        // diag_D[k] = D_kk * L_kk * U_kk
        diag_D[k] = gf256CtxPtr->gf256_mul(D_kk, gf256CtxPtr->gf256_mul(L_kk, U_kk));
        ...
        for (int j = k + 1; j < N; ++j)
        {
            ...
            // L_jk = g[j] / (x_j + y_k)
            // U_kj = b[j] / (x_k + y_j)
            const uint8_t L_jk = gf256CtxPtr->gf256_div(g[j], gf256CtxPtr->gf256_add(x_j, y_k));
            const uint8_t U_kj = gf256CtxPtr->gf256_div(b[j], gf256CtxPtr->gf256_add(x_k, y_j));
            ...
            *row_U++ = U_kj;
            ...
            g[j] = gf256CtxPtr->gf256_mul(g[j], gf256CtxPtr->gf256_div(gf256CtxPtr->gf256_add(x_j, x_k), gf256CtxPtr->gf256_add(x_j, y_k)));
            b[j] = gf256CtxPtr->gf256_mul(b[j], gf256CtxPtr->gf256_div(gf256CtxPtr->gf256_add(y_j, y_k), gf256CtxPtr->gf256_add(y_j, x_k)));
        }
        ...
        gf256CtxPtr->gf256_div_mem(row_L, row_L, L_kk, count);
        gf256CtxPtr->gf256_div_mem(rotated_row_U, rotated_row_U, U_kk, count);
    ...
    for (int j = N - 1; j > 0; --j)
    {
        ...
        gf256CtxPtr->gf256_mul_mem(row_U, row_U, gf256CtxPtr->gf256_add(x_0, y_j), count);
        row_U += count;
    }
    ...
    const uint8_t U_nn = gf256CtxPtr->gf256_mul(b[N - 1], gf256CtxPtr->gf256_add(x_0, y_n));

    // diag_D[N-1] = L_nn * D_nn * U_nn
    diag_D[N - 1] = gf256CtxPtr->gf256_div(gf256CtxPtr->gf256_mul(L_nn, U_nn), gf256CtxPtr->gf256_add(x_n, y_n));
}
...
void ReedSolomonDecoder::Decode()
{
    ...
    for (int originalIndex = 0; originalIndex < OriginalCount; ++originalIndex)
    {
        ...
        for (int recoveryIndex = 0; recoveryIndex < N; ++recoveryIndex)
        {
            ...
            gf256CtxPtr->gf256_muladd_mem(outBlock, matrixElement, inBlock, Params.BlockBytes);
        }
    }
    ...
    */
    ...
    for (int j = 0; j < N - 1; ++j)
    {
        ...
        for (int i = j + 1; i < N; ++i)
        {
            ...
            gf256CtxPtr->gf256_muladd_mem(block_i, c_ij, block_j, Params.BlockBytes);
        }
    }
    ...
    */
    for (int i = 0; i < N; ++i)
    {
        ...
        gf256CtxPtr->gf256_div_mem(block, block, diag_D[i], Params.BlockBytes);
    }
